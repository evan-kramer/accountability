...
print("3-8 loaded (4/9)")
print(lubridate::now())

cdf <- bind_rows(fall_eoc, spring_eoc, tn_ready) %>%
  mutate(
    ri_status = ifelse(reason_not_tested == 1 & ri_status == 6, 0, ri_status),
...
print("Student-level initiated (6/9)")
print(lubridate::now())
...
print("Deduplication complete (7/9)")
print(lubridate::now())
...
student_level <- dedup %>%
  ...
  filter(!(state_student_id == 4244992 & subject == "ELA"))
